...
describe("ModelFilter", () => {
  ...
  describe("matchesPattern", () => {
    ...
    it("should match wildcard in middle", () => {
      ...
      expect(matchesPattern("openai/gpt-3.5-turbo", "openai/gpt-*-turbo")).toBe(
        true,
      );
  ...
  describe("filterModels", () => {
    ...
    it("should apply denylist after allowlist", () => {
      ...
      expect(first.id).toBe("openai/gpt-4");
    });
  });
  ...
  describe("property-based model type filtering", () => {
    ...
    const typeArb = fc.option(
      fc.constantFrom("chat", "language", "embedding"),
      {
        nil: undefined,
      },
    );
    ...
    it("is idempotent", () => {
      fc.assert(
        fc.property(modelsArb, (models) => {
          ...
          const filteredTwice = transformModels(
            models.filter(isAllowedType),
          ).map((model) => model.id);
  ...
  describe("configuration change handling", () => {
    it("should update filters when configuration changes", () => {
      ...
      const filtered = filter.filterModels(mockModels);
      expect(filtered[0]).toBeDefined();
      const first = filtered[0];
      if (!first) {
        throw new Error("Expected filtered model to be defined");
      }
      expect(first.id).toBe("openai/gpt-4");
    });
  });
