...
describe("parseModelIdentity", () => {
	...
	it("extracts provider for colon-separated ids", () => {
		...
		const providerArb = fc.stringOf(segmentChar, { minLength: 1 });
		const modelPartArb = fc.stringOf(segmentChar, { minLength: 0 });
	...
	it("strips version suffixes from family", () => {
		...
		const familyArb = fc
			.stringOf(familyChar, { minLength: 1 })
		...
		fc.assert(
			fc.property(familyArb, versionArb, separatorArb, (family, version, separator) => {
	...
	it("defaults version to latest when no suffix matches", () => {
		...
		const modelPartArb = fc.stringOf(safeChar, { minLength: 1 });
