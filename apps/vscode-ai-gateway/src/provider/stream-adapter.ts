...
export class StreamAdapter {
  ...
  private handleResponseCompleted(
    ...
    if (!outputItems) {
      logger.debug(
        `[OpenResponses] response.completed has no output array (id=${response?.id ?? "unknown"})`,
      );
    } else {
      const typeCounts = new Map<string, number>();
      for (const item of outputItems) {
        const t = (item as { type?: string }).type ?? "unknown";
      ...
      const topTypes = Array.from(typeCounts.entries())
        ...
        .join(", ");
      logger.debug(
        `[OpenResponses] response.completed output summary (id=${response?.id ?? "unknown"}, len=${outputItems.length}, types=${topTypes})`,
      );
    }
    ...
    if (outputItems) {
      ...
      if (functionCallsFound > 0) {
        logger.info(
          `[OpenResponses] Completion output tool calls: found=${functionCallsFound}, emitted=${functionCallsEmitted}, skippedDupe=${functionCallsSkippedDuplicate}`,
        );
      }
    }
  ...
  private handleOutputItemAdded(
    ...
    if (!item) {
      logger.debug(
        `[OpenResponses] output_item.added: item is null (output_index=${event.output_index})`,
      );
      return { parts: [], done: false };
    }
    ...
    const itemType = "type" in item ? (item as { type: string }).type : "no-type";
    logger.debug(
      `[OpenResponses] output_item.added: type=${itemType}, keys=[${itemKeys.join(", ")}], output_index=${event.output_index}`,
    );
    ...
    if ("type" in item && (item as { type: string }).type === "function_call") {
      const fcItem = item as { type: string; id?: string; call_id?: string; name?: string };
      const callId = fcItem.call_id ?? "";
      const name = fcItem.name ?? "";
      const id = fcItem.id ?? "";
    ...
    } else if (itemType !== "message" && itemType !== "reasoning" && itemType !== "no-type") {
  ...
  private handleOutputItemDone(
    ...
    const itemType =
      item && "type" in item ? (item as { type: string }).type : "no-type";
    ...
    const isFunctionCallByType =
      item && "type" in item && (item as { type: string }).type === "function_call";
    ...
    if (isFunctionCallByType || isFunctionCallByProps) {
      const callId = (item as { call_id: string }).call_id;
      const argsStr = (item as { arguments: string }).arguments;
      const name = (item as { name: string }).name;
  ...
  private handleContentPartAdded(
    ...
    logger.debug(
      `[OpenResponses] content_part.added: type=${partType ?? "null"}, item_id=${event.item_id}, content_index=${event.content_index}`,
    );
  ...
  private handleFunctionCallArgsDone(
    ...
    logger.debug(
      `[OpenResponses] function_call_arguments.done: itemId=${itemId}, argsLen=${finalArguments?.length ?? "n/a"}`,
    );
    ...
    logger.warn(
      `[OpenResponses] function_call_arguments.done received but no matching function call found for itemId=${itemId}. ` +
        `Tracked function calls: ${Array.from(this.functionCalls.entries()).map(([k, v]) => `${k}:${v.itemId}`).join(", ") || "none"}`,
    );
