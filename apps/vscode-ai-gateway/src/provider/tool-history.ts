...
export class ToolHistoryManager {
	...
	addToolCall(
		...
		this.logger.trace(
			`[ToolHistory] Added ${name} (${category}): ${tokenCount} tokens, error=${isError}`,
		);
	}
	...
	getCompactedHistory(): CompactedHistory {
		...
		const recentCount = Math.min(this.config.recentCallsToKeep, this.history.length);
		...
		const recentCalls = recentEntries.map((entry) => this.formatFullEntry(entry));
   ...
   */
			...
			if (errors.length > 0) {
				...
				for (const error of errors) {
					...
					const truncatedResult =
						error.result.length > 200 ? `${error.result.slice(0, 200)}...` : error.result;
	 ...
	 */
	private summarizeCategory(category: ToolCategory, entries: ToolCallEntry[]): string {
		...
		switch (category) {
			case "read": {
				...
				return `- Read ${nonErrorEntries.length} file(s): ${pathPreview}${more}`;
			}
				...
				const paths = [
					...new Set(
						nonErrorEntries.map((e) => {
							const path = (e.args as { filePath?: string }).filePath || "unknown";
							return path;
						}),
					),
				];
				return `- Made ${nonErrorEntries.length} edit(s) to: ${paths.slice(0, 3).join(", ")}${paths.length > 3 ? "..." : ""}`;
			}
			...
			case "terminal": {
				...
				return `- Ran ${nonErrorEntries.length} terminal command(s): ${commands.slice(0, 2).join("; ")}${commands.length > 2 ? "; ..." : ""}`;
			}
			...
			case "search": {
				...
				return `- Performed ${nonErrorEntries.length} search(es): ${queries.slice(0, 2).join(", ")}${queries.length > 2 ? "..." : ""}`;
			}
			default: {
				const tools = [...new Set(nonErrorEntries.map((e) => e.name))];
				return `- Used ${nonErrorEntries.length} other tool(s): ${tools.slice(0, 3).join(", ")}${tools.length > 3 ? "..." : ""}`;
			}
		}
	}
	 ...
	 */
	private groupByCategory(entries: ToolCallEntry[]): Map<ToolCategory, ToolCallEntry[]> {
	 ...
	 */
	private estimateTokens(name: string, args: Record<string, unknown>, result: string): number {
		...
		const totalChars = name.length + JSON.stringify(args).length + result.length;
 ...
 */
function generateTruncationSummary(entries: ToolCallEntry[], config: TruncationConfig): string {
	if (entries.length === 0) return "";
 ...
 */
function groupByCategory(entries: ToolCallEntry[]): Map<ToolCategory, ToolCallEntry[]> {
	const groups = new Map<ToolCategory, ToolCallEntry[]>();
 ...
 */
function summarizeCategoryEntries(category: ToolCategory, entries: ToolCallEntry[]): string {
	const nonErrorEntries = entries.filter((e) => !e.isError);
	if (nonErrorEntries.length === 0) return "";
