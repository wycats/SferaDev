...
type MessagePart =
  vscode.LanguageModelChatMessage["content"] extends Iterable<infer T>
    ? T
    : never;
...
export class TokenCache {
  ...
  private serializeParts(parts: IterableLike<MessagePart>): unknown[] {
    return Array.from(parts as Iterable<MessagePart>, (part) =>
      this.serializePart(part),
    );
  }

  private serializePart(part: unknown): unknown {
  ...
  private isToolResultLike(
    part: unknown,
  ): part is { callId?: unknown; content: unknown } {
    return (
      typeof part === "object" &&
      part !== null &&
      "content" in part &&
  ...
  private serializeToolResultContent(content: unknown): unknown {
    ...
    if (this.isIterable(content)) {
      return Array.from(content, (item) =>
        this.serializePart(item as MessagePart),
      );
    }
    return content;
  }
