  ...
  implements AuthenticationProvider, Disposable
{
  ...
  async getSessions(
    ...
    if (needsUpdate) {
      await this.context.secrets.store(
        SESSIONS_SECRET_KEY,
        JSON.stringify(refreshedSessions),
      );

      // Fire session change event for refreshed tokens
      if (changedSessions.length > 0) {
        const changedAuthSessions: AuthenticationSession[] =
  ...
  async createSession(
    ...
    return authMethod === "oidc"
      ? this.createOidcSession()
      : this.createApiKeySession();
  }
  ...
  async removeSession(sessionId: string): Promise<void> {
    ...
    if (activeSessionId === sessionId) {
      ...
      const newActiveSession = firstSession ? firstSession.id : null;
      ...
      if (newActiveSession) {
        ...
        window.showInformationMessage(
          `Activated: ${newActive.account.label} ${methodLabel}`,
        );
      }
    }
  ...
  private async promptForSessionName(): Promise<string | undefined> {
    return window.showInputBox({
      ...
      ignoreFocusOut: true,
      validateInput: (value) => this.validateSessionName(value),
    });
  }
  ...
  private generateSessionId(): string {
    return `${EXTENSION_ID}-${Date.now().toString()}-${Math.random().toString(36).substring(2, 11)}`;
  }
  ...
  private async promptForAuthMethod(): Promise<
  ...
  private async promptForAction(
    ...
    const methodLabel = activeSession
      ? this.getMethodLabel(activeSession.method)
      : "";
  ...
  private async handleRemoveSession(sessions: SessionData[]): Promise<void> {
    if (sessions.length === 1) {
      ...
      await this.removeSession(onlySession.id);
      return;
    }
  ...
  async getActiveSession(): Promise<SessionData | null> {
    ...
    if (activeSession.method === "oidc" && activeSession.oidcData) {
      try {
        ...
        logger.error(
          "Failed to refresh OIDC token in getActiveSession:",
          error,
        );
  ...
  private getActiveSessionId(): Promise<string | null> {
    return Promise.resolve(
      this.context.globalState.get(ACTIVE_SESSION_KEY, null),
    );
  }
  ...
  private async switchActiveSession(): Promise<void> {
    ...
    const methodLabel = selectedSession
      ? this.getMethodLabel(selectedSession.method)
      : "";
    ...
    window.showInformationMessage(
      `Switched to: ${String(selectedSession?.account.label)} ${methodLabel}`,
    );
  }
}
