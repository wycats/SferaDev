...
export function extractCreatorAndModel(
  modelId: string,
): { creator: string; model: string } | null {
...
export class ModelEnricher {
  ...
  private loadFromPersistence(): void {
    ...
    try {
      const persisted =
        this.globalState.get<PersistedEnrichmentCache>(ENRICHMENT_CACHE_KEY);
      if (persisted?.version === 1) {
        ...
        logger.debug(
          `Restored ${this.cache.size.toString()} enrichment cache entries from storage`,
        );
      }
    } catch (error) {
  ...
  async enrichModel(
    modelId: string,
    apiKey: string,
  ): Promise<EnrichedModelData | null> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.warn(
          `Enrichment endpoint failed for ${modelId}: HTTP ${response.status.toString()} ${response.statusText}`,
        );
        return null;
      }
      ...
      await this.persistToStorage();
      logger.info(
        `Enriched ${modelId} in ${(Date.now() - startTime).toString()}ms`,
      );
      return data;
    } catch (error) {
