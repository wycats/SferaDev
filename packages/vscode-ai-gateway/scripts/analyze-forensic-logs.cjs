...
if (!fs.existsSync(logPath)) {
  ...
  console.error(
    `Please ensure forensic logging is enabled in provider.ts and you have generated some activity.`,
  );
  process.exit(1);
}
...
entries.forEach((entry) => {
  ...
  if (!currentBurst || ts - lastTs > 1500) {
    // 1.5s window
    currentBurst = {
      start: entry.ts,
      strings: [],
...
console.log(
  `Analyzing ${entries.length} log entries in ${bursts.length} evaluation bursts...`,
);
...
bursts.forEach((burst, index) => {
  ...
  const messageTokensDeduped = Array.from(uniqueMessages.values()).reduce(
    (a, b) => a + b,
    0,
  );
  ...
  if (totalDeduped > 100) {
    // Filter out tiny checks
    console.log(`\nBurst #${index + 1} at ${burst.start}`);
    console.log(
      `  Tools (Strings): ${stringCount} calls, ${stringTokens} tokens`,
    );
    console.log(
      `  Messages:        ${uniqueMessages.size} unique (${messageCount} calls), ${messageTokensDeduped} tokens`,
    );

    const limit =
      burst.strings[0]?.maxInput || burst.messages[0]?.maxInput || 128000;
    ...
    console.log(
      `  Overhead Ratio:  ${((stringTokens / totalDeduped) * 100).toFixed(1)}% tools`,
    );
  }
});
