...
export function createClient(options: ClientOptions) {
		...
		try {
			...
			if (!response.ok) {
				...
				const errorMsg = parsed.error?.param
					? `${parsed.error?.message ?? `HTTP ${response.status}`} (param: ${parsed.error.param})`
					: (parsed.error?.message ?? `HTTP ${response.status}`);
				throw new OpenResponsesError(errorMsg, response.status, parsed.error?.code, parsed);
			}
	...
	async function* createStreamingResponse(
		...
		for await (const event of parseSSEStream(response.body)) {
			eventCount++;
			trace(`[OpenResponses] SSE event #${eventCount}`, event);
			yield event;
		}
		debug(`[OpenResponses] Stream completed with ${eventCount} events`);
	}
	...
	for (const line of lines) {
		if (line.startsWith("data: ")) {
			...
			try {
				...
				console.warn(`[OpenResponses] Failed to parse SSE data: ${e instanceof Error ? e.message : String(e)}. Data: ${preview}`);
			}
		}
...
export function isResponseCompleted(event: StreamingEvent): event is ResponseCompletedEvent {
	return event.type === "response.completed";
}

export function isResponseFailed(event: StreamingEvent): event is ResponseFailedEvent {
