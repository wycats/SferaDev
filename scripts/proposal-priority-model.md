...
| **Correctness** | Fixes bugs or eliminates fragile assumptions. Split into _fragile_ (relying on undocumented behavior that could break any release) vs _imprecise_ (heuristic that's good enough). | System prompt role detection, stateful-marker DataPart persistence |
...
- **Entangled in design** → factor into interface design _now_, even without adopting
...
**Integration gets its own fast lane.** Integration items are the _only_
...
any release is _risk_. Risk gets priority regardless of timeline.

**Core-entangled features produce "design awareness", not "lower priority".**
You don't need to _adopt_ `chatSessionsProvider` today. But if your
...
> **v0 integration context:** Several proposals below (chatParticipantAdditions, toolProgress, chatHooks, chatOutputRenderer) form a cluster that would enable native tool rendering if the extension registers as a `ChatParticipant`. This is a significant scope increase sketched in **RFC 00071**. The gradings below evaluate each proposal on its own merits; see RFC 00071 for the v0-specific integration story.
...
| 60    | **toolProgress**                   | Feature (orthogonal) | `ToolProgressStep` adds progress bars (message + increment) inside tool invocation UI. Zero consumers today (not even copilot-chat), no version tag. Complements chatParticipantAdditions tool rendering — that covers non-progress-bar messaging via `invocationMessage`/`pastTenseMessage`; this adds discrete progress bars specifically. Relevant if extension registers `LanguageModelTool` instances (see RFC 00071).                                                           |
| 60    | **chatHooks**                      | Feature (orthogonal) | Shell command hooks at chat lifecycle points: `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `PreCompact`, `SubagentStart`, `SubagentStop`, `Stop`. Version 6, copilot-chat@6 = actively iterated. `PreToolUse`/`PostToolUse` hooks provide a user-configurable safety/governance layer for tool execution. `ChatResponseHookPart` renders hook results (including `stopReason` for blocking) in the stream. Relevant if extension registers tools (see RFC 00071). |
...
| 51    | **chatOutputRenderer**                | Feature (orthogonal) | Custom webview renderers for tool result MIME types via `registerChatOutputRenderer()`. Enables rich rendering of tool output (dashboards, previews, etc.) instead of plain text. Still has 3 TODOs, zero consumers, and long-term horizon — genuinely early. Relevant if extension registers tools with rich output (see RFC 00071). |
